...
export default function DiscoverPage() {
  const filters = [
    ...
    { label: "Disaster Relief", Icon: Anchor },
    ...
    <div>
      ...
      <div className="container" style={{ paddingTop: 32, paddingBottom: 60 }}>
        ...
        <div style={{ background: "linear-gradient(135deg,rgba(74,155,106,.08),rgba(61,122,82,.08))", border: "1px solid rgba(74,155,106,.2)", borderRadius: "var(--r)", padding: "20px 24px", marginBottom: 28, display: "flex", alignItems: "center", gap: 20 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, color: "var(--forest)", fontSize: 16 }}>Campaigns Near You — Metro Manila</div>
            <div style={{ fontSize: 13, color: "var(--text2)", marginTop: 4 }}>Showing {CAMPAIGNS.length} verified campaigns</div>
          </div>
          <button className="btn btn-outline btn-sm" style={{ marginLeft: "auto", flexShrink: 0 }}>Change Location</button>
        </div>

        <div className="grid-4 mb-32">
          <div className="stat-card"><div className="stat-value">{CAMPAIGNS.length}</div><div className="stat-label">Active Campaigns</div><div className="stat-change">All verified</div></div>
          <div className="stat-card"><div className="stat-value">₱{(CAMPAIGNS.reduce((s, c) => s + c.raised, 0) / 1000).toFixed(0)}K</div><div className="stat-label">Total Raised</div><div className="stat-change">↑ Growing daily</div></div>
          <div className="stat-card"><div className="stat-value">100%</div><div className="stat-label">Verified Campaigns</div><div className="stat-change">↑ All screened</div></div>
        ...
        <div className="flex flex-center flex-between mb-24">
          <h3 style={{ fontSize: 20, fontWeight: 700, color: "var(--forest)", display: "flex", alignItems: "center", gap: 8 }}>
            <TrendingUpIcon /> Active Campaigns
          </h3>
        ...
        <div className="campaign-grid">
          {CAMPAIGNS.map((c) => {
            return (
              <Link key={c.id} href={`/detail?id=${c.id}`} className="camp-card" style={{ textDecoration: "none" }}>
                <div className="camp-img" style={{ background: c.heroGradient }}>
                  <div className="camp-img-inner" style={{ fontSize: 48 }}>{c.heroIcon}</div>
                  {c.urgencyLabel && (
                  ...
                  <div style={{ position: "absolute", bottom: 12, left: 12, right: 12, background: "rgba(26,58,42,.75)", borderRadius: 8, padding: "8px 12px" }}>
                    ...
                    <div style={{ height: 4, background: "rgba(255,255,255,.2)", borderRadius: 2, marginTop: 4 }}>
                      <div style={{ width: `${c.transparencyScore}%`, height: 4, background: "var(--canopy)", borderRadius: 2 }} />
                    </div>
                  </div>
                </div>
                <div className="camp-body">
                  ...
                  <div className="prog-track" style={{ height: 8 }}>
                    <div className="prog-fill prog-emerald" style={{ width: `${c.pct}%` }} />
                  </div>
                  <div className="camp-footer">
                    <span className="badge badge-navy">{c.institutionDesc}</span>
                    <span style={{ fontSize: 12, color: c.accentColor, fontWeight: 600 }}>{c.pct}% · {c.daysLeft}d left</span>
                  </div>
                </div>
              </Link>
            );
