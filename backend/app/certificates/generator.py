...
def generate_certificate_pdf(
    ...
    story.append(
        Paragraph("has made a verified, blockchain-recorded donation of", body_style)
    )
    ...
    story.append(
        Paragraph(f"<b>Tx Hash:</b> {stellar_tx_hash[:32]}...", footer_style)
    )
    story.append(
        Paragraph(
            f"<b>Date:</b> {donation_date.strftime('%B %d, %Y at %H:%M %Z')}",
            footer_style,
        )
    )
